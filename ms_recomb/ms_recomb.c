	...
	while( arg < argc ){
		...
		switch ( argv[arg][1] ){
			...
			case 'I' :
				...
				// MPC - code not currently configured for multiple populations
				fprintf(stderr,"Code not currently configured for use with multiple populations.\n");
				exit(1);
			...
			case 'e' :
				...
				case 'N' :
				      ...
				      break;
				/* variable recombination over time -2-Mar-10 */
				case 'R' :
			          argcheck( arg, argc, argv);
				 	  pt->paramv = atof( argv[arg++] ) ;
				      break;
				/* end variable recombination addition */
				case 'G' :
...
usage()
{
fprintf(stderr,"usage: ms_recomb nsam howmany \n");
...
fprintf(stderr,"\t -eR t rho  (Modify recombination rate of all pop's.)\n");
